...
test_dataset = RecDataset(file_list=glob.glob("../HumanData/1ch/**/**/**/**.png"), transform=trans)
...
if dev == "cpu":
    ...
    net = torch.load(load_path + "flow/model-50-epoch")
...
for inputs, flows, img_numbers, notes, mic in test_loader:
    ...
    result.append([mic[0], img_numbers.item(), flows[0], outputs[0].item()*0.1+0.5, notes.item(), preds.item()])
